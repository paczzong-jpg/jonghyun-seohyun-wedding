// Copy to api/resend-email.pb.js.
// Sends email through Resend using the MISO runtime proxy.

routerAdd("POST", "/api/email/resend/send", function (e) {
  ...
  try {
    ...
    var apiKey = runtimeEnv.RESEND_API_KEY || "";
    var from = clampString(body.from || runtimeEnv.RESEND_FROM_EMAIL, 500);
    ...
    if (!apiKey) return e.json(500, { error: "Missing RESEND_API_KEY" });
    if (!from || !to || !subject || (!html && !text)) {
      return e.json(400, { error: "from, to, subject, and html or text are required" });
    }

    var payload = { from: from, to: [to], subject: subject };
    ...
    return e.json(200, { ok: true, id: data && data.id ? data.id : "", data: data });
  } catch (err) {
